...
export async function requireAuth(req,res,next){
  try{
    ...
    const user=await queryOne('SELECT id,name,email,role,can_export,can_edit,can_delete,active FROM users WHERE id=? AND deleted_at IS NULL',[dec.id]);
...
export function enrichUser(u){
  return{...u,es_admin:u.role==='ADMIN',puede_editar:u.role==='ADMIN'||!!u.can_edit,puede_eliminar:u.role==='ADMIN'||!!u.can_delete,puede_exportar:u.role==='ADMIN'||!!u.can_export};
}
